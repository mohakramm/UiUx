    ...
    <header className="fixed inset-x-0 top-0 z-50 border-b border-line bg-ink/80 backdrop-blur-md">
      <nav aria-label="Main" className="mx-auto flex h-16 max-w-6xl items-center justify-between px-5 sm:px-8">
        <Link href="/" className="group flex items-baseline gap-2">
          ...
          <span className="hidden text-[0.7rem] uppercase tracking-[0.18em] text-muted transition-colors group-hover:text-gold sm:inline">
            UI/UX & Product
          </span>
        </Link>
      ...
      {/* Mobile menu */}
      {open && (
        <ul id="mobile-menu" className="border-t border-line bg-ink px-5 py-3 md:hidden">
          {links.map((link) => (
            <li key={link.href}>
              <Link
                href={link.href}
                ...
                className={`block rounded-lg px-3 py-3 text-base ${
                  isActive(link.href) ? "text-gold" : "text-paper-dim hover:text-paper"
                }`}
              >
                {link.label}
              </Link>
